  ...
  // Выравнивание картинок аккордеона — по краю самого файла (прозрачные поля не компенсируем)

  // Правило раскрытия таблицы: десктоп (≥1280) — все разделы раскрыты,
  // планшет/мобила — только первый. Применяется при загрузке и смене брейкпоинта,
  // ручные клики внутри одного режима не перетираются.
  (function(){
    var table = document.getElementById('cmpTable');
    if (!table) return;
    var lastMode = null;
