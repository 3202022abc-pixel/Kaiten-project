...
/**
 * FormMessengers — блок «регистрация в один клик через мессенджер» для форм
 * event/webinar-лендингов. Отдельный элемент завода (по образцу `FormConsent`):
 * разделитель + кнопки Telegram и MAX, ведущие в ботов регистрации.
 *
 * Стандарт ТОЛЬКО для вебинаров (правило `webinar-messenger-registration`): на
 * других типах лендингов форму мессенджерами не дополняем. Рендерится **opt-in** —
 * кнопка появляется лишь когда задан её deep-link бота; если не задано ни одной
 * ссылки, компонент не рендерит ничего (можно ставить в форму безусловно).
 *
