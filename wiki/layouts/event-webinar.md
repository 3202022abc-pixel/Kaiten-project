...
- **Мессенджер-регистрация (только вебинары).** Форму можно дополнить кнопками
  «в один клик через мессенджер» — отдельный элемент `FormMessengers` (Telegram +
  MAX, ведут в ботов регистрации). Ссылки ботов — в `form.telegramHref` /
  `form.maxHref` (и в `RegistrationCta`); рендер opt-in — нет ссылки, нет кнопки,
  заглушка `#` рисует плейсхолдер. Стандарт **только для `event_landing`**, не для
  форм других типов лендингов. Правило: `webinar-messenger-registration`.
