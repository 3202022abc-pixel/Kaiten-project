...
- **Форма — стандартный вид, без мессенджеров.** По умолчанию форма регистрации —
  поля + `FormConsent`, без кнопок мессенджеров (эталон `webinar-vnedrenie-kaiten`
  — именно такой). Кнопки Telegram/MAX (`FormMessengers`) — **второй, исключительный
  вариант** формы (не только для вебинаров), включается opt-in через
  `form.telegramHref` / `form.maxHref` (нет ссылок — нет кнопок). Правило:
  `messenger-form-variant`.
